...
- 并额外生成一个统一总包目录：`factory/flagship-complete`
...
- 你想得到一个把 20 个旗舰全放在一起的统一目录

### `npm run build:flagship-complete`

作用：只重建统一的 20 旗舰总包，不分 batch 看。

什么时候需要：
- 你只关心完整的 20 个旗舰总包
- 你不想在 `flagship-batch-1` 到 `flagship-batch-4` 之间来回切
- 你想直接打开一个目录就看到全部旗舰 agent 和三端运行时包
